...
let loadingUiText = "<center>$[tada.speed=0s $[position.x=1.7 :sasayaki_akari:]]]</center>"
Ui:render([
  Ui:C:mfm({ text: loadingUiText })
])

let array = CUSTOM_EMOJIS.map(@(emoji) { emoji.name }).filter(@(name) { name.index_of("sasayaki") != -1 })
...
// 結果のテキスト
let result = `<small>今日のささやきは〜？</small>
