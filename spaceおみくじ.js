...
let loadingUiText = "<center>$[tada.speed=0s $[position.x=1.7 :nowloading:$[position.x=-1.7 :utyuu:]]]</center>"
Ui:render([
  Ui:C:mfm({ text: loadingUiText })
])

let array = CUSTOM_EMOJIS.map(@(emoji) { emoji.name }).filter(@(name) { name.index_of("space") != -1 })
